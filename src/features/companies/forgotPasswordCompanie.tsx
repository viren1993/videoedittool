...
const resetPasswordSchema = z.object({
  password: z
    ...
    .regex(new RegExp(".*\\d.*"), {
      message: "The password must contain at least one numerical character",
    })
...
export default function ForgotPasswordCompanie({
  data: accoutdata,
  setRefreshApi,
}: CompanieDeleteProps) {
  ...
  const onSubmit: SubmitHandler<updatedPasswordInput> = async (data) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setRefreshApi((prev: boolean) => !prev);
        setOpen(false);
      } else {
  ...
  if (!accoutdata) return null;

  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger>
        <IconButton color="violet" variant="soft">
          <KeySquare width="18" height="18" />
        </IconButton>
      </Dialog.Trigger>
